...
contract FreePair is IFreePair {
    ...
    function pushAddr(uint256 pushAddrProposalId, address[] memory addrs) public virtual override returns (bool) {
    ...
    function depositRequest(string memory senderOnSideChain) public virtual override returns (address) {
        ...
        return depositAddr[index]; // todo test to add when init and fuction
    }
    
    // 每个代币的小数点的统计！！
    function depositConfirm(uint256 mintProposalId, string memory senderOnSideChain, uint256 amount, string memory txOnSideChain) public virtual override returns (bool) {
        require(iFreeFactory.notaryMap(msg.sender), "depositConfirm can only be called by notary");
        require(migrated == false, "this pair has migrated to pair v2, please use pair v2");
        address sender = senderInfoMap[keccak256(bytes(senderOnSideChain))].sender;
