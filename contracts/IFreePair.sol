...
interface IFreePair {
    ...
    function depositConfirm(string memory txOnSideChain, string memory senderOnSideChain, uint256 amountOnSideChain, uint256 amount) external returns (bool);
    function withdrawRequest(address sender, string memory recipientOnSideChain, uint256 amount) external returns (bool);
    function withdrawConfirm(uint256 burnProposalId, uint256 amountOnSideChain, string memory txOnSideChain) external returns (bool);
